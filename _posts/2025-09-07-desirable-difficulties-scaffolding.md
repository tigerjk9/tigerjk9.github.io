---
title: "AI는 '바람직한 어려움'과 '스캐폴딩'의 완벽한 균형을 찾을 수 있을까?"
date: 2025-09-07 00:14:00 +0900
categories: [AI, 학습과학]
tags: [학습과학, 인지부하이론, 적응형학습, 스캐폴딩, 바람직한어려움]
---

## 인공지능(AI)을 통해 바람직한 어려움과 스캐폴딩은 적절한 균형을 찾을 수 있을까?
...
​
<br>**인지 부하 이론(CLT)을 통한 조화의 프레임워크**
...
​
<br>**<인지부하 최적화 교육전략 매트릭스 / CLT 기반 적응적 교수법 매트릭스>**
...
다음 표는 본 장에서 논의된 핵심 개념들을 인지 부하 이론의 관점에서 비교하여 요약한 것입니다.

##### 표 1. 교육적 마찰과 지원의 비교 프레임워크
| 개념 | 주요 목표 | 대상 학습자 | 핵심 메커니즘 | 인지 부하 이론과의 관계 |
|:---|:---|:---|:---|:---|
| **바람직한 어려움** | 장기 기억 및 전이 강화 | 중급자 ~ 전문가 | 인출 연습, 분산, 교차, 생성 | 내재적/외재적 부하가 낮을 때 유의미 부하 증가 |
| **교육적 스캐폴딩** | ZPD 내 과제 수행 가능화 | 초보자 | 과제 분해, 모델링, 힌트, 템플릿 | 내재적 부하가 높을 때 외재적 부하 감소 |
| **파괴적 인지 마찰 (HCI)** | (없음 - 디자인 결함) | 모든 학습자 | 비일관적 UI, 혼란스러운 정보 구조 | 학습 목표와 무관한 외재적 부하 증가 |
...
다음 표는 본 장에서 논의된 주요 AI 기술들을 기능과 교육적 목표에 따라 정리한 것입니다.

##### 표 2. 적응형 학습 시스템을 위한 AI 기술
| AI 기술/기법 | 핵심 기능 | 교육적 목표 | 예시 적용 |
|:---|:---|:---|:---|
| **베이지안 지식 추적 (BKT)** | 은닉 마르코프 모델로 기술 숙달 확률 추정 | 학습자의 현재 지식 상태를 모델링하여 다음 과제 추천 | 간단한 기술 기반 학습 시스템 |
| **딥러닝 지식 추적 (DKT)** | RNN을 사용하여 학습자의 상호작용 이력을 기반으로 잠재적 지식 상태 모델링 | 미래 수행 능력을 더 정확하게 예측하여 과제 선택 및 스캐폴딩 정보 제공 | 복잡한 상호 의존성을 가진 지식 도메인 |
| **감성 컴퓨팅** | 얼굴 표정, 음성, 생체 신호 등으로 학습자의 감정 상태(예: 좌절, 지루함) 감지 | 동기 부여 관리 및 이탈 방지를 위한 적시 개입(예: 격려, 난이도 조절) | 몰입형 학습 시뮬레이션 |
| **다중 모드 학습 분석 (MMLA)** | 로그, 비디오, 오디오 등 다양한 데이터를 통합하여 학습 과정에 대한 총체적 이해 구축 | 인지적, 정서적, 행동적 측면을 종합적으로 고려한 정교한 학습자 모델링 | 협력 학습 분석 시스템 |
| **동적 난이도 조절 (DDA)** | 학습자 수행 능력에 맞춰 실시간으로 과제 난이도 조정 | 학습자를 '플로우' 상태에 유지하여 몰입과 지속적인 참여 유도 | 게임 기반 학습 환경 |
| **LLM 기반 대화 에이전트** | 자연어 대화를 통해 소크라테스식 질문, 개인화된 설명, 피드백 생성 | 처방적 피드백을 넘어, 학습자의 사고를 유도하고 깊은 이해를 촉진 | 대화형 AI 튜터 |
| **자동화된 계획 생성 (LLM)** | 학습자의 행동 로그를 분석하여 다음 학습 목표와 계획을 텍스트로 생성 | 자기 조절 학습(SRL) 기술, 특히 목표 설정 및 계획 수립 능력 지원 | 개방형 학습 환경(OELE) |

이 표는 교육 기술 설계자들이 사용 가능한 'AI 툴킷'을 체계적으로 이해하고, 구축하고자 하는 특정 적응형 기능에 어떤 기술이 필요한지를 판단하는 데 도움을 줄 수 있습니다.
