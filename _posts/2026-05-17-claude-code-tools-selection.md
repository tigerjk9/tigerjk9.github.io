...
다섯 번째 도구는 Vercel에서 만든 **react best practices** 스킬이다. 리액트 코드를 작성할 때 Vercel 팀이 정의한 모범 사례를 클로드 코드에 적용한다. 기본 클로드 코드는 리액트 컴포넌트를 잘 작성하지만, 유지 이펙트 남용이나 클라이언트-서버 컴포넌트 구분 미흡 등 아쉬운 부분이 있을 수 있다. 이 스킬은 서버에서 처리해야 할 부분을 알려주거나, 더 이상 사용되지 않는 패턴을 지적하는 등 코드 품질을 개선한다.
