...
categories: [AI, 기술]
tags: [워터마크, AI탐지, 클로드, SynthID, AI윤리, 콘텐츠출처, EUAI법, 저작권]
description: "앤스로픽이 클로드가 만든 모든 글에 보이지 않는 워터마크를 심기로 했다. 출처를 새기려는 창과 그 표식을 지우는 방패가 맞붙는다. 워터마크의 원리와 약점, 그리고 이 대결의 본질을 짚는다."
permalink: /post/ai-watermark-vs-remover/
---
앤스로픽이 자사 AI 클로드가 만든 모든 글에 사람 눈에 보이지 않는 워터마크를 심기로 했다. 복사해 붙여넣거나 일부를 고쳐도 표식이 일부 남는다. 8월 2일 이후 나온 모델부터 적용하고, 이전 모델에도 순차로 넣는다. 유럽연합 AI법의 투명성 의무에 따른 조치이고, 전 세계 이용자에게 똑같이 적용된다.
