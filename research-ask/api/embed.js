...
import { applyCors, rateLimit, requireKey, userGeminiKey, handleByokError, embedQuery } from '../lib/store.js';

export default async function handler(req, res) {
  if (applyCors(req, res)) return;
  if (req.method !== 'POST') return res.status(405).json({ error: 'method_not_allowed' });
  if (requireKey(req, res)) return;
  const byokKey = userGeminiKey(req); // 방문자 본인 키 — 임베딩 비용이 방문자 부담이라 일일 총량 제외
  if (rateLimit(req, res, 10, 1200, Boolean(byokKey))) return;
  ...
  try {
    const vec = await embedQuery(q, byokKey);
