...
<style>
  ...
  #rh-ai-toggle {
    ...
    padding: 0.55em 0.9em; cursor: pointer; font-family: inherit; transition: all .16s;
    white-space: nowrap;
  }
  ...
  @media (max-width: 600px) {
    ...
    #rh-app .rh-hero { gap: 10px; }
    #rh-app .rh-hero h1 { font-size: 1.45rem; }
...
(function () {
  ...
  // 주인장 전용 접근 키 — /ask/ 페이지에서 입력하면 같은 localStorage를 공유
  ...
  function ensureVectors() {
    ...
    return fetch('/assets/research-emb-posts.json')
    ...
    fetch(ASK_API + '/api/health', { signal: ctrl ? ctrl.signal : undefined, headers: keyHeaders() })
      ...
      .then(function (h) {
        if (h && h.ok && h.hasKey && (!h.authRequired || h.authorized)) {
          ...
          document.getElementById('rh-ask-cta').style.display = 'inline-flex';
        }
      })
