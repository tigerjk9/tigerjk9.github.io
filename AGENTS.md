...
`/wrap` — 작업 세션 마무리 전역 스킬 (`~/.Codex/skills/wrap/SKILL.md`).
PRD 정리 → AGENTS.md 정리 → 메모리 저장 → git commit & push → **전역 자산 동기화** → /compact 안내 6단계를 순서대로 실행한다.

5단계(전역 자산 동기화)는 `~/.Codex`의 스킬·에이전트·커맨드를 비공개 저장소 `tigerjk9/Codex-config`로 밀어 **다른 머신에서도 잡히게** 한다(2026-09-09 추가). 프로젝트 저장소 push는 `~/.Codex`를 건드리지 않아, 한 머신에서 만든 전역 스킬이 wrap을 해도 다른 머신에 전달되지 않는 문제가 있었다. `~/.Codex/.gitignore`가 `/*`로 전부 무시한 뒤 `skills/`·`agents/`·`commands/`·`AGENTS.md`만 여는 허용목록 방식이라 **새로 만드는 전역 스킬은 자동으로 대상에 포함**되고, 자격증명·세션·메모리는 구조적으로 올라가지 않는다. 대량설치분(gstack 555MB 포함 스킬 92·에이전트 19)과 외부 클론 `hwpx-skill`은 제외 — 새 머신은 `~/.Codex/bootstrap.ps1`로 받는다.

**새 머신 온보딩 (2026-09-10, 이 머신에 적용)**: `bootstrap.ps1`이 저장소 안에 있어 닭-달걀이다. 먼저 `gh repo clone tigerjk9/Codex-config <임시경로>` 한 뒤 **그 사본의** `bootstrap.ps1`을 실행한다 — 스크립트가 `~/.Codex`에서 `git init` → `fetch` → `reset`(인덱스만) → **로컬에 없는 파일만** `checkout` 한다. 비파괴적이라 로컬이 더 최신인 파일은 덮어쓰지 않고 목록만 보고한다(이 머신은 OMC 4.13.7 `AGENTS.md`·`_workspace` 회전 규칙이 든 `orchestrator-template.md` 2건이 로컬 우선이었다). 실행에서 걸린 함정 넷:
...
앞의 둘은 `bootstrap.ps1`에 수정 반영했다(Codex-config `a367a64`).

### 슬래시 커맨드 카탈로그

블로그 자동화·유지보수 슬래시 커맨드 전체 목록. 상세는 각 커맨드 파일(`.Codex/commands/<name>.md`)과 아래 해당 섹션 참고.
...
| `/tidy-Codex-md` | AGENTS.md 진단·정리(6지표 채점, 교훈 보존) | `tidy-Codex-md.md` |
| `/wrap` | 세션 마무리(PRD→AGENTS.md→메모리→커밋→전역 동기화) | 전역 `~/.Codex/skills/wrap/SKILL.md` |
...
| 지도 도법 교육 웹앱 (메르카토르↔Equal Earth·실제 크기 겹쳐보기) | `tools/map-projection/index.html`(+`vendor/` self-host), `scripts/map-projection-prd.md` |
...
**`tools/` 정적 단독 페이지**: front matter 없는 `tools/<슬러그>/index.html`은 Jekyll이 정적 파일로 그대로 복사한다(exclude 목록에 없음). 테마 CSS·`sidebar-toggle.js`가 로드되지 않아 라이트모드 앵커 색·플로팅 ☰ 버튼 함정이 원천 차단된다 — layout default 커스텀 페이지에 필요한 `#앱ID` 스코핑도 불필요. 첫 사례가 NE 수업 디자이너(`/tools/ne-designer/`), 두 번째가 어휘 사전(`/tools/vocab/`), 세 번째가 지도 도법 웹앱(`/tools/map-projection/` — D3+vendor/ self-host, `?embed=1`로 Equal Earth 글에 iframe 삽입·postMessage 자동 높이, 실제 크기 겹쳐보기 오버레이). 검증은 스크래치패드 복사 후 Edge 헤드리스 `#demo` 해시 렌더 캡처(메모리 `project_tools_static_pages`·`project_map_projection_tool`). **모바일 검증은 window-size 최소폭 아티팩트 때문에 390px iframe 하네스로 캡처**(SVG 오버레이 라벨은 viewBox 단위라 좁은 화면에서 작아짐 → `@media(max-width:480px) .ovlab` 폰트 상향). 한글 줄바꿈 고아 방지는 `word-break:keep-all`. 데이터 fetch가 절대경로(`/assets/...`)면 로컬 file:// 에서 안 잡히므로 `window.fetch`를 목데이터로 덮어쓴 사본을 만들어 캡처한다.
...
Codex에서는 `/paper <PDF경로>` 슬래시 커맨드로 호출한다 (`.Codex/commands/paper.md`).
...
- **표·가독성·어미 후처리 (필수, 2026-09-13 배치 점검 정착)**: /paper 배치 산출물에서 반복되는 4종 결함을 Python 스윕으로 점검한다(Git Bash grep은 UTF-8 콜론 미탐 → Python). 상세·검증 스캐너는 메모리 `feedback_paper_batch_readability_endings_qa`. ① **빈 셀 표** — Gemini가 첫 열을 비워(`| | ...`) 병합을 흉내 → 마크다운 빈 셀로 깨짐. 카테고리 값을 각 행에 반복 채움, 2계층 중첩표(집단 인간/LLM을 값에 끼운 것)는 자립 열 구조로 재구성, 라벨 문장 바로 밑 표는 빈 줄 삽입(없으면 미렌더). 대시 40+ 폭발 플래그는 파이프 유지 시 false positive. **줄 앞 공백이 있는 들여쓴 표(`^ +\|`)는 kramdown이 코드블록으로 통째 깨뜨린다 — `.strip()` 후 파이프만 보는 파서가 놓치니 선행 공백을 반드시 확인**. "(N) 라벨 + 4칸/8칸 들여쓴 본문" 구조 글(vibe-coding류)은 섹션 전체를 dedent(4칸→0·8칸→2칸 중첩·표 열0), 표가 열0 리스트 중간에 끼면 표 뒤 4칸 줄이 코드블록이 되므로 평탄화. ② **열거 벽글** — "(3) …아이디어… 첫째/둘째/셋째"를 한 줄(500~900자)에 몰아쓴 것 → 인트로 문장 + 빈 줄 + `- 첫째/둘째/셋째` 불릿으로 분해(단어 변경 없음). `~어렵고, 둘째` 접속 한 문장·이미 `-`불릿 내부 정의는 보존. ③ **탐구질문 비의문형 어미** — `무엇임?`·`존재함?`·`필요함?`·`~질문?`처럼 물음표는 있으나 서술형/명사구 → 의문형(`무엇인가?`·`~하는가?`), 카테고리 라벨의 오류 `?`는 제거. 검출은 물음표 유무가 아니라 `?` 앞 음절이 가/까/나/니인지. ④ **명사형/서술형 혼입** — 명사형(~함/됨/임) 글에 `~이다`/`~하다`/`~목표다`가 섞이면 명사형으로 정렬. 어미·존칭 판정 전 **인용문 제거 필수**(예시 프롬프트·AI 튜터 발화의 존칭은 정당). ⑤ **`*   ` 들여쓰기 하위목록 run-on** — "(N) 소제목" 문단 뒤 빈 줄 없이 `    *   역할:`·`*   논문 예시:` 하위목록을 붙이면 kramdown이 리스트로 못 읽고 한 줄로 흘림 → 문단 뒤 빈 줄 + `- ` 불릿 전환(라벨 볼드). 단 앞에 빈 줄 있는 `*` 리스트나 `-`상위+`*`중첩은 정상이니 유지(검출: 문단 다음 줄이 `^\s*\*\s`). 수정 후 `build_research_db.py`→`build_embeddings.py` 재생성.
...
.Codex/commands/*.md    # 슬래시 커맨드 (전체 목록은 상단 "슬래시 커맨드 카탈로그" 표 참고)
```
...
Codex에서는 `/paraph <URL>` 슬래시 커맨드로 호출한다 (`.Codex/commands/paraph.md`).
...
수동 실행은 `/digest` 슬래시 커맨드 (`.Codex/commands/digest.md`).
...
- **후처리 QA**: 링크가 대상 포스트 permalink와 일치하는지(지어낼 수 있음), S1 금지 표현(`~을 넘어` 등 혼입 확인됨), 포스트 누락, 존칭 어미 — `.Codex/commands/digest.md`의 체크리스트 참고
...
Codex에서는 `/cardnews <입력>` 슬래시 커맨드로 호출 (`.Codex/commands/cardnews.md` — QA 체크리스트 포함).
...
- **후처리 QA**: cards.json 사실성 대조·이미지 육안 확인 필수 (`.Codex/commands/cardnews.md` 체크리스트). **논문 출처(source_label)는 저자·연도·제목을 원문에서 verbatim 확인 — 요약·의역해 짧은 제목 지어내기 금지, arXiv/DOI는 추출값만**(hook-image 스킬과 동일 환각방지 원칙, 2026-08-01)
- **밝은 다이어그램 스타일 (`--style diagram`, 2026-08-02)**: 다크 시네마틱과 별개 경로. 밝은 크래프트지 배경 + 손그림 개념 다이어그램 1장. **다이어그램은 이미지 생성이 아니라 SVG로 그려 한글이 안 깨진다**(이미지 모델 한글 파손 회피 — hook/cardnews가 텍스트를 오버레이하는 것과 같은 이유). 기존 `_shot`(Edge 캡처)·`_font_css` 재사용, 과금은 스펙 카피 1회뿐. 입력은 `--topic "주제"` 직접 또는 URL/PDF 추출 **둘 다**.
  ...
  - **전역 스킬화 (2026-08-02)**: `~/.Codex/skills/cardnews/SKILL.md`로 어느 cwd에서든 사용(엔진은 블로그 repo 절대경로 호출, 리소스는 `__file__` 기준 해석이라 cwd 무관 — `_load_dotenv`도 `REPO_ROOT/.env`). 엔진·브랜드 자산(logo·.env·.fonts)이 블로그 repo에 묶여 그 저장소가 있어야 동작. 전역 스킬은 스타일 선택→아키타입→생성 워크플로 포함. 프로젝트 커맨드(`.Codex/commands/cardnews.md`)는 in-repo 상대경로로 병존.
...
Codex에서는 `/naver` 슬래시 커맨드로 호출한다 (`.Codex/commands/naver.md` — 옵션·QA·운영 규칙 상세).
...
- **이력은 '발행했다는 기록'이지 '올라갔다는 증거'가 아니다 — `--audit` (2026-09-02, 누락 12편 사고 후 추가)**: 중복 발행만 막아 왔지 **반대편(누락)을 확인하는 장치는 없었다**. 발행 후 URL을 못 잡으면 `url="unknown"`으로 이력에 적었는데, 그 값은 '다른 주소로 올라갔다'가 아니라 '올라갔는지 모른다'다. 한 번 적히면 영원히 대상에서 빠지므로 실제로 발행되지 않은 글이 조용히 사라진다 — 2026-08-31 04:45~04:55 배치 10편 + 2026-08-18 1편 + logNo가 존재하지 않는 1편, **총 12편이 '게시 완료'로 위장된 채 누락**돼 있었다(이력 346편 중). 게다가 발행 전 guard는 *이력에 없는 글*에만 걸려서 유령 기록은 어느 경로로도 드러나지 않았다. → ① `write_post`가 URL을 못 잡으면 `confirm_published()`가 목록 API를 8초 간격 3회 다시 읽어 실재를 확인하고, **확인될 때만** 이력에 남긴다(아니면 `[UNVERIFIED]`로 두고 다음 실행에 재시도) ② `py -u scripts/naver_crosspost.py --audit`이 네이버 전체 글과 이력·`_posts/`를 대조해 **①유령 기록 ②진짜 미게시 ③이력 누락**으로 분류 출력(로그인 불필요·9초), `--fix-ledger`로 유령 기록을 지워 재발행 대상으로 되돌린다. 오조회로 이력을 날리지 않도록 **색인 100편 미만이거나 유령이 이력의 20% 초과면 중단**(`--force`로 강행). **`/naver` 실행 전 `--audit`을 습관화**할 것 — 이력만 보면 늘 '싱크 완료'로 보인다
- **제목은 발행 이후에도 바뀐다 (guard 오탐 원인)**: `_norm_title`이 물음표를 구분해서, 2026-08-02 의문문 제목 물음표 일괄 통일(38편)로 **블로그 제목만** 바뀐 12편은 네이버에 멀쩡히 있는데도 guard가 못 찾았다(이력이 유실되면 그대로 중복 발행). 정규화에서 `?`·`!`를 지워 해결. 제목 색인도 최근 900편(30페이지)에서 **전체**(2026-09 기준 2557편, 9초)로 확장했다 — 900편 창 밖의 오래된 글은 대조 자체가 불가능했다. 접두 20자 매칭은 유지: 실측한 접두 전용 매칭 13건이 전부 진짜 제목 드리프트(부제 추가·의문형 어미 교체)였고 오탐은 0건이었다
- **아카이브 백필 `--backfill` (2026-09-02)**: 크로스포스팅 시작(baseline, 2026-05-14) 이전 271편 중 **99편이 네이버에 없다**. 전수 확인 결과 빠진 이유가 선별이 아니라 **수동 크로스포스팅이 못 따라간 것** — 누락이 글 단위가 아니라 날짜 단위로 뭉쳐 있다(2025-10-06 하루 8편 통째 누락인데 그달 다른 38편은 게시됨, 2026-02-28 4편 전부, 2025-11은 20편 중 18편). 본문 중앙값 3,479자에 논문리뷰·에세이가 대부분이라 상록수 콘텐츠로서 지금 올려도 값을 한다. → `--backfill`이 baseline 제한을 풀되 **스케줄 태스크는 이 플래그가 없어 신규 글만 다룬다**(`BASELINE_FILENAME`을 통째로 내리면 스케줄러가 하루 30편으로 아카이브를 붓는다). `BACKFILL_EXCLUDE` 8편(인사글 198자·출간 '예정' 안내·546~953자 개발 알림 등 시효 지난 공지)을 빼고 **91편**이 대상. **저속 기본값이 자동 적용된다** — `--limit 8 --daily-cap 10 --min-wait 60 --max-wait 120`(명시값 우선). 1년치를 하루 30편으로 부으면 정확히 네이버가 스팸으로 보는 몰아쓰기가 된다. 수동 시절 올린 172편은 이력에 없어서 `drop_already_on_naver()`가 미리 걸러내고 이력을 보정한다 — **안 걸러내면 batch 앞자리를 전부 차지하고 guard가 하나씩 건너뛰느라 실제 발행이 0편**이 된다(`batch = pending[:limit]`이라 건너뛴 만큼 채워지지 않음). 백필 시 신규 글이 아카이브에 밀려 굶지 않도록 baseline 이후를 앞으로 정렬한다
...
- **`url: "unknown"`은 "다른 주소로 발행됨"이 아니라 "발행 결과 미확인"**: 실제로 발행이 성사되지 않은 기록일 수 있다(2026-09-02 실측으로 확정 — `unknown` 11편 전부 네이버에 없었다. **이제 코드가 이 값을 쓰지 않는다**. 확인 실패는 `[UNVERIFIED]`로 두고 이력에 남기지 않는다. 남아 있는 `unknown` 행은 그 이전 기록이다). 2026-08-03 이 값을 중복 근거로 읽어 **유일본을 삭제**했다(재발행 복구). 중복 판정에서 남길 쪽 URL이 `unknown`인 행은 제외한다(`warn_unknown_pair`)
...
Codex에서는 `/yeonsu <입력>` 슬래시 커맨드로 호출한다 (`.Codex/commands/yeonsu.md`).
...
황민호 수석 KIST Codex 워크숍 자료(`260429_황민호_강의자료.Zip`)를 첫 사례로 강의자료 zip 한 묶음(slides·instructor-notes·handout·labs·N 기능 카탈로그) → `_lectures/` Jekyll collection 자동 큐레이션. 5명 Superpowers 멀티 에이전트 팀(inventory·parser·curator·builder·reviewer).
...
| D 첫 변환·큐레이션 (수동) | `Codex-edu`: 허브 + 22 feature 페이지 + 슬라이드·핸드아웃·OG 커버. 김진관/닷커넥터 큐레이션 메타 + K-12 교사 듀얼 트랙 (15장 슬라이드 + 6곳 핸드아웃) + 허브 카드 큐레이션 배지·크레딧 + Reveal 캔버스 1280x820 + 핸드아웃 터미널 카드 재설계 | ✅ |
| C 자동화 진입점 | `.Codex/skills/lecture-archive-orchestrator/SKILL.md` + `.Codex/commands/lecture-archive.md` (두 번째 강의부터 적용 예정) | ⏳ |

**큐레이션 작업 패턴 (Codex-edu에서 정착)**
...
- **외부 기관명 de-institutionalization**: 슬러그·타이틀에서 KIST 등 제3자 기관명 회피 (`Codex-edu`). 청중명(교육자)로 치환
...
- 도서 1~7권은 `tigerjk9/Book-Publisher`(비공개 레포) 완성 원고 — 전부 라이브 Vercel 웹 도서로 새 탭 연결(웹 도서가 단일 진실 소스, 원고 개정 시 Vercel만 재배포하면 블로그는 무수정. 1권은 teacher-Codex-guide.vercel.app — 최초 편입 때 배포 없는 줄 알고 블로그 내 전문 렌더링했다가 사용자가 URL 확인해 줘 제거).
- **8권(2026-07-21)은 한빛미디어 신간(요즘 교사를 위한 웹앱 만들기 with 바이브 코딩 · 서명 확정 2026-08-24 · 이상선·김진관·김상섭·이대형·윤신영 공저)으로 유일하게 블로그 내부 큐레이션 정리본(`_lectures/vibecode-for-teacher/`)에 연결** — 페이지 본문은 목차·파트별 요약·실습 표·컴패니언 GitHub(lifeofpi-ux/vibecode-for-teacher) 중심이고, **4교지 수정 원고(2026-08-26 · 145쪽 · 43MB)는 릴리스 태그 `vibecode-for-teacher-book`** 자산(`vibecode-for-teacher-proof4-2026-08-26.pdf`)으로 올려 '원고 내려받기' 섹션에서 링크한다 — 저장소에 커밋 금지. 목차 표기는 4교지 목차를 따른다(로컬 환경 / React / 8장 '개발부터 배포까지, 내 손으로 완성하기').
...
- **Subagent 환경 Ruby 부재**: Codex Agent dispatch subagent에는 `bundle`·`ruby`·`jekyll` PATH 없음. Jekyll 변경 검증은 사용자 측 직접 빌드로.
...
3. **미번역 영단어·영문장** — 한국어로 교체. 단 툴·플러그인·API 식별자(`useEffect`·`Superpowers`·`MCP`·`AGENTS.md` 등)는 영문 유지가 정답이며, 한글 음역됐으면 영문으로 복구한다.
