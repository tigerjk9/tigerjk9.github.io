...
## `/edit-paraph` 변형 — 블로그 주인장 목소리 강화

`web_to_post.py --edit` + `edit_web_prompt_template.txt` 사용.

**추가된 3층 서술 원칙** (각 섹션 필수):
1. **원문 주장** — 저자가 실제로 주장한 것. 표현을 바꾸되 논지는 정확하게 재현
2. **근거·사례** — 원문의 수치·비유·예시·사례를 그대로 활용. 근거 없는 추론 금지
3. **필자의 판단** — 주장의 타당성, 흔들리는 지점, 한국 교육 현장 적용 가능성

**필자 관점 (필수)**: 직접 판단(`솔직히`, `개인적으로는`) + 절제된 비관 + 수사학적 의심 최소 2곳.
독자에게 한 마디를 거는 문장(질문·권유·경고) 최소 1곳.

**차이점**: `/paraph`는 전문가 어투 재서술, `/edit-paraph`는 필자 목소리·비판적 관점 강화.
