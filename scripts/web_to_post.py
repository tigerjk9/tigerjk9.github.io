...
# google-generativeai가 이 속성을 사용하므로 없으면 빈 dict 반환 shim 삽입
try:
...
from image_fetcher import fetch_and_inject_image, inject_permalink, normalize_question_title, normalize_title_dash, get_existing_taxonomy, CROSSOVER_DOMAINS, replace_image_markers, fetch_og_image_url, download_image  # noqa: E402
...
def main() -> None:
    ...
    # 소스 이미지 사전 수집 (입력 URL의 OG 이미지)
    _source_images: list[Path] = []
    print("[INFO] 소스 이미지 사전 수집 중...")
    for _i, _url in enumerate(args.urls):
        _og_url = fetch_og_image_url(_url)
        if _og_url:
    ...
    markdown_content = normalize_title_dash(markdown_content)
