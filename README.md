...
**URL을 여러 개 전달하면 하나의 통합 포스트**로 종합한다.

```bash
python scripts/web_to_post.py <URL>                          # 단일 출처 변환 + git push
python scripts/web_to_post.py <URL1> <URL2>                  # 복수 출처 통합 + git push
...
Claude Code에서는 `/paraph <URL> [URL2 ...]` 스킬로 호출한다.
...
5. 20개 분야 풀에서 랜덤 크로스오버 분야 선택
6. Gemini(`gemini-2.5-flash`)로 한국어 포스트 생성 (단일 / 복수 출처 통합)
7. `_posts/YYYY-MM-DD-{slug}.md` 저장 → git commit + push
...
scripts/
  web_to_post.py              # /paraph — 웹 아티클 → 포스트 변환 (복수 URL 통합 지원)
