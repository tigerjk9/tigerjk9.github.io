...
**고정 섹션 구조**: 연구 목적 → 연구 방법 → 주요 발견 → 결론 → ADD One → 탐구 질문 + APA 출처  
Figure 자동 추출(PyMuPDF, 최대 6개) 후 멀티모달 삽입.
...
- **분량**: 원본 내용을 빠짐없이 다룬다. 생략 없음.
- **구조**: 도입부 → 본문(자유 섹션) → 크로스오버 → 출처
- **크로스오버**: 실행마다 20개 분야 풀(신경과학·행동경제학·언어학·음악이론 등)에서 랜덤 선택
