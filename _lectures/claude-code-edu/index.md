...
| # | 기능 | 핵심 메시지 |
|---|------|------------|
| 1 | CLAUDE.md + @import | 매 세션 시스템 프롬프트 복붙을 0으로 |
| 2 | Auto Memory | 내가 정정한 걸 다음 세션에 기억. 내 디스크 마크다운으로 보관 |
| 3 | Plan Mode | 변경 전 계획만 받는 안전망 모드 |
| 4 | Permission Modes (6종) | AI 자율성을 다이얼로 조절 |
| 5 | Skills (`/skill-name`) | 한 번 정의 후 한 줄 호출 |
| 6 | Subagents | 전문 역할 AI를 분리해 병렬 운용 |
| 7 | `/agents` Library/Running | 서브에이전트 컨트롤 패널 |
| 8 | `/compact` · `/clear` | 컨텍스트 압축과 초기화 |
| 9 | `/usage` | 토큰·비용·세션 통계 |
| 10 | `--continue` · `--resume` | 끊긴 세션 한 줄에 부활 |
| 11 | Agent Teams | 다수 에이전트 협업 (실험 기능) |
| 12 | `.claude/rules/` + paths 글롭 | 토픽별 룰 자동 로드 |
...
| # | 기능 | 핵심 메시지 |
|---|------|------------|
| 1 | 헤드리스 모드 (`-p` + JSON) | 셸 한 줄로 AI를 파이프라인에 |
| 2 | Hooks (라이프사이클) | 30+ 이벤트에 자동 핸들러 부착 |
| 3 | MCP | GitHub·Slack·DB 등 외부 시스템 연결 |
| 4 | Plugins | 스킬·에이전트·훅을 한 패키지로 배포 |
| 5 | Routines (클라우드 영구) | 노트북이 꺼져도 도는 무인 작업 |
| 6 | `/ultrareview` · `/ultraplan` | 다중 에이전트 병렬 검토·계획 |
| 7 | `/loop` | 한 프롬프트를 인터벌마다 반복 |
| 8 | `/schedule` | 자연어로 cron 등록 |
| 9 | `/effort` | 추론 강도 5단계 조절 |
| 10 | UX 보조 5종 | `/powerup` · `/focus` · `/copy` 등 |
