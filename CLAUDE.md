...
- **카피**: `line1`+`line2`를 이어 읽어 한 문장, 끊는 지점에 긴장. **각 행 8자 내외**(길면 축소돼 힘이 빠짐). `validate_copy`가 존칭체·훈계조·AI티·행 길이·콜론을 잡고 최대 2회 재작성. 카드뉴스의 "`~다` 종결" 규칙은 **적용하지 않는다** — 후킹은 의문형이 가장 강해서다(레퍼런스가 `공부일까, / 연기일까?`)
