...
- **만료 전이면 `--force-login`이 사람 입력 없이 갱신된다 (2026-09-16 실측, 1회)**: D-1(만료 09-18) 상태에서 `--force-login`을 돌렸더니 **아이디·비밀번호를 한 글자도 입력하지 않고** 12초 만에 새 `NID_AUT`가 발급되고 만료가 10-16로 리셋됐다. 프로필에 살아 있는 세션이 로그인 페이지에서 그대로 재인증되고, `apply_keep_login`이 '로그인 상태 유지'를 체크한 덕이다. 즉 **사람 손이 필요한 건 세션이 이미 죽은 뒤뿐**이고, 살아 있는 동안은 갱신을 자동화할 수 있다(예: 월 1회 D-7 예약). 단 아직 1회 관측이라 다음 갱신 때 한 번 더 확인할 것 — 재현되면 위 ②(재방문 시 재발급 안 됨)는 '폼 방문 + 상태유지 체크'에는 해당되지 않는 것으로 정리한다
