...
- **`--force-login`의 무인 갱신은 가짜였다 (2026-09-16, 같은 날 반증)**: D-1 상태에서 `--force-login`을 돌리자 자격증명 입력 없이 12초 만에 새 `NID_AUT`가 발급되고 만료가 30일 뒤로 찍혔다. 스크립트도 `로그인 확인 완료`를 냈다. **그런데 2시간 뒤 16:00 스케줄 실행이 `[EXPIRED]`로 죽었다** — 쿠키 파일은 D-30인데 서버가 거부했다(`--check-session`으로 재확인). 즉 **쿠키의 만료 표기는 증거가 아니다**([[feedback_ledger_is_not_proof]]와 같은 함정). 살아 있는 세션으로 로그인 폼을 치면 네이버가 새 쿠키를 주지만 그게 유효한 세션이라는 보장이 없고, 오히려 기존 세션을 무효화할 수 있다. **월 1회 무인 갱신 자동화 계획은 폐기한다.** 30일마다 사람이 실제로 아이디·비밀번호를 입력해야 한다. 갱신 후에는 반드시 `--check-session`으로 **서버 기준** 확인을 한 번 더 한다 — 스크립트의 성공 메시지만 믿지 말 것
...
- **대량 이관 (2026-09-16, 3차에 걸쳐 `assets/` 930MB → 379MB)**: 이미지 축소 225개(-126MB) + 릴리스 이관 3묶음. 태그 `student-question-assessment`(35개·221MB) · `ai-leading-teacher-2026`(18개·160MB) · `lecture-materials`(9개·45MB). 매 단계 저장소 내 문서 링크 실존 확인 + 라이브에서 페이지·릴리스 다운로드 검증.
- **`gh release upload`는 파일 basename을 자산 이름으로 쓴다**: 강의마다 `slides.pdf` 같은 같은 이름을 써서 한 릴리스에 모으면 충돌한다. 계획 단계에서만 이름을 바꾸면 URL과 실제 자산이 어긋나고, 같은 basename 둘을 한 번에 올리면 **404로 업로드 자체가 실패**한다. 도구가 임시 폴더에 계획된 이름으로 복사해 올린다. 업로드 실패 시 원본을 지우지 않는 순서라 되돌릴 것이 없다.
- **잠금 카드 페이지의 자료는 옮기지 않는다**: `ai-off-for-20-minutes`의 PDF 3개(15MB)는 페이지가 `pw-gate` 암호화 payload라 내려받기 링크가 그 안에 있을 수 있다. 참조 여부를 확인할 방법이 없으면 이관 대상에서 뺀다.
- **`_lectures/student-question-assessment/index.md`의 워크시트 HWP 4개는 원래부터 깨진 링크**다(git 히스토리에 없음). 이관과 무관하며 원본 파일을 받으면 릴리스에 올려 살릴 수 있다.
