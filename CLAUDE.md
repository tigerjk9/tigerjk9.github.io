...
## 강의 스크립트 자동화 (`/lecture-script`)

`scripts/lecture_script.py`가 다양한 입력(YouTube/웹/PDF/파일)을 교사 연수용 강의 스크립트로 자동 변환한다.
Claude Code에서는 `/lecture-script <입력>` 슬래시 커맨드로 호출한다 (`.claude/commands/lecture-script.md`).
...
### 출력 구조 (슬라이드 연계 강의 원고 + 블로그 포스트)

각 `## [슬라이드 N] 제목` 섹션:
- `**핵심 내용**` — 슬라이드에 들어갈 불릿
- `**강의 원고**` — 구어체 대본 (인용 블록, 분당 250~300자)
- `**발문**` — 교사 성찰 질문 (전체 3~5개만)
