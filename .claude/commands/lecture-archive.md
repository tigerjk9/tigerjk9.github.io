...
강의자료 zip 한 묶음(slides·instructor-notes·handout·labs·기능 카탈로그)을 tigerjk9.github.io 블로그의 `_lectures/` collection으로 자동 큐레이션한다.
...
| 옵션 | 설명 |
|------|------|
| `<zip-path>` | 강의자료 zip 절대 경로 (필수) |
| `--slug` | 강의 슬러그 강제. 생략 시 zip 파일명에서 자동 추론 |
...
`$ARGUMENTS`가 비어 있으면 zip 경로를 먼저 사용자에게 물어본 후 아래를 실행한다.
